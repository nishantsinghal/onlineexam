	...
	/**  FACULTY FUNCTIONALITY      **/
	  ...
	  function create_test1() {
	  	...
	  	$subid = $_POST['sub_id'];
			...
			while($resul = mysql_fetch_array($re)){
				$lid    = $resul['l_id'];
				$quer1  = "select * from level where l_id=$lid";
				$re1    = mysql_query($quer1,$conn);
				$res    = mysql_fetch_array($re1);
				$ques["$i"] = array('subid'=>$resul['sub_id'],'qbid'=>$resul['qb_id'],'question'=>$resul['question'],'marks'=>$resul['marks'],'level'=>$res['level']);
				$i++;
			}
